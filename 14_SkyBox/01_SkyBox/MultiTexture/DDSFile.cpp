...
bool DDSFile::LoadCubeMap(const char* filePath[])
{
	glGenTextures(1, &_textureId);
	glBindTexture(GL_TEXTURE_CUBE_MAP, _textureId);
	for (auto i = 0; i < 6; ++i) {
		int channels = 0;
		_imageData = SOIL_load_image(filePath[i], &_imageWidth, &_imageHeight, &channels, SOIL_LOAD_RGB);
		if (_imageData == nullptr) {
			MessageBox(NULL, "载入文件失败!", "错误", MB_OK);
			return false;
		}
		glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, GL_RGB, _imageWidth, _imageHeight, 0, GL_RGB, GL_UNSIGNED_BYTE, _imageData);
		SOIL_free_image_data(_imageData);
	}
