...
void MultiTexture::setupLights(glm::vec3* PointLightPositions, int pointLightCnt)
{
	...
	glUniform3f(glGetUniformLocation(_pShaderProgramHandle, "u_dirLight.direction"), lampDir.x, lampDir.y, lampDir.z); // 方向光源
	// 设置点光源
	for (auto i = 0; i < pointLightCnt; ++i) {
		// 设置光源属性 点光源
		std::stringstream indexStr;
		indexStr << i;
		std::string lightName	= "u_pointLights[" + indexStr.str() + "]";  // 使用数组索引pointLights[i]
