...
bool MultiTexture::Init()
{
	...
	_specularTexture.Load("container_specular.bmp");
	if (CompileShaderProgram(sphere_vert, sphere_frag, &_pShaderProgramHandle) 
		/*&& CompileShaderProgram(lamp_vert, lamp_frag, &_pLampProgramHandle)*/) {
...
void MultiTexture::Draw()
{
	...
	glUniform3f(glGetUniformLocation(_pShaderProgramHandle, "u_light.direction"), 0.5f, 0.8f, 0.0f); // 方向光源
	// 启用diffuseMap
	...
	glm::mat4 view			= glm::lookAt(glm::vec3(1.2f, 1.5f, 4.5f), glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3(0.0f, 1.0f, 0.0f));
